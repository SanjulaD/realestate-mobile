  ...
  return (
    <View className="flex flex-col items-start">
      <View className="flex flex-row items-center">
        ...
        <Text className="ml-3 text-start font-rubik-bold text-base text-black-300">{item.name}</Text>
      </View>

      <Text className="mt-2 font-rubik text-base text-black-200">{item.review}</Text>
      ...
      <View className="mt-4 flex w-full flex-row items-center justify-between">
        ...
        <Text className="font-rubik text-sm text-black-100">{new Date(item.$createdAt).toDateString()}</Text>
      </View>
    </View>
  )
}
